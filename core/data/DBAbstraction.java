...
public interface DBAbstraction
{
	 ...
	 * 
	 * @param path The path to the sqlite file that contains all the data.
	 */
	...
	/**
	 * Adds directory to the database, taking all relevant info needed to recreate directory and stuffing it in the database.
	 * Be sure to complement this with an insertEvent(CreateDocumentEvent event)
	 * @param node
	 */
	...
	/**
	 * Gets a single Developer from the database based on the first and last name.
	 * @param firstName
	 * @param lastName
	 * @return
	 * 
	 * Deprecated by Kevin 10/31/2012 because we should now identify developers by email address, not by name
	 */
	/*
	@Deprecated
	public Developer getDeveloperByName(String firstName, String lastName) throws DBAbstractionException;
	*/
	/**
	...
	/**
	 * Gets all events that have the createdUnderNodeId of nodeID starting at sequenceNumber for a length of blockSize.
	 * Does not do any sort of node familial computation, i.e. if blockSize is bigger than the number of events in this node,
	 * this will not wrap over.  That is/should be done in StorytellerServer (with PlaybackNodes).
	 * @param nodeID
	 * @return
	 */
	...
	/**
	 * Gets all the insert events in a document
	 * @param docID The document that where we are looking for inserts
	 * @return A list of insert events
	 * @throws DBAbstractionException
	 */
	//public List < InsertEvent > getAllInsertEventsInADocument(String docID) throws DBAbstractionException;
	
	/**
	 * Gets the entire state of the file system at a node/sequence number.
	 * 
	 * @param nodeID The node where to get the state of the file system
	 * @param sequenceNum The sequence number within the node to get the state
	 * @throws DBAbstractionException
	 */
	//public ExtendedDirectory getTheStateOfTheFileSystem(String nodeID, int sequenceNum, boolean withEventIds) throws DBAbstractionException;

	/**
	 * Gets the entire state of the file system at a node/sequence number.
	 * 
	 * @param nodeID The node where to get the state of the file system
	 * @param idOfEventToStopAtWithinANode In place of a sequence number this is the id of the event to stop at
	 * @throws DBAbstractionException
	 */
	//public ExtendedDirectory getTheStateOfTheFileSystem(String nodeID, String idOfEventToStopAtWithinANode, boolean withEventIds) throws DBAbstractionException;

	/**
	 * Gets the entire state of the file system at a node.
	 * 
	 * @param nodeID The node where to get the state of the file system, it will go to the 
	 * latest event in the node
	 * @param sequenceNum The sequence number within the node to get the state
	 * @throws DBAbstractionException
	 */
	//public ExtendedDirectory getTheStateOfTheFileSystem(String nodeID, boolean withEventIds) throws DBAbstractionException;
	
	/**
	 * Get all the documents in the system at a given node and sequence number
	 * @param nodeID Node where we are looking for the documents
	 * @param sequenceNum Sequence number within the node to look for the documents
	 * @return A list of all documents at the point in time
	 * @throws DBAbstractionException
	 */
	//public List<ExtendedDocument> getAllDocumentsUpToAPointInTime(String nodeID, int sequenceNum, boolean withEventIds) throws DBAbstractionException;
	
	/**
	 * Get all the documents in the system at a given node and sequence number
	 * @param nodeID Node where we are looking for the directories
	 * @param sequenceNum Sequence number within the node to look for the directories
	 * @return A list of all directories at the point in time
	 * @throws DBAbstractionException
	 */
	//public List<ExtendedDirectory> getAllDirectoriesUpToAPointInTime(String nodeID, int sequenceNum) throws DBAbstractionException;
	
	/**
	 * Gets the parent directory of a directory at a point in time. Directories can be moved so this
	 * will find the latest move directory and get the parent directory id.
	 * 
	 * @param id
	 * @param nodeID
	 * @param sequenceNum
	 ...
	 * @throws DBAbstractionException
	 */
	//public String getParentDirectoryOfDirectoryAtAPointInTime(String dirId, String nodeID, int sequenceNum) throws DBAbstractionException;
	
	/**
	 * Gets the parent directory of a document at a point in time. Documents can be moved so this
	 * will find the latest move document and get the parent directory id.
	 * 
	 * @param id
	 * @param nodeID
	 * @param sequenceNum
	 ...
	 * @throws DBAbstractionException
	 */
	//public String getParentDirectoryOfDocumentAtAPointInTime(String docId, String nodeID, int sequenceNum) throws DBAbstractionException;
		
	/**
	 * Get the contents of a document at a point in time
	 * 
	 * @param docId id of the document
	 * @param nodeID node to look in
	 * @param sequenceNum max seq num to look into
	 * @return A string with the contents of a file
	 * @throws DBAbstractionException
	 */
	//public String getContentsOfADocumentUpToAPointInTime(String docId, String nodeID, int sequenceNum, boolean withEventIds, List < InsertEvent > eventIds) throws DBAbstractionException;
	
	/**
	 * Gets the name of a document at a point in time. Documents in the database do not have a name they
	 * have an id. Names are set by either the create document event or the rename document event. This 
	 * method gets the name at a certain point in time.
	 * 
	 * @param nodeID The node to look in
	 * @param sequenceNum The sequence number in the selected node
	 * @return The name of the document at a point in time
	 * @throws DBAbstractionException
	 */
	//public String getDocumentNameAtAPointInTime(String docId, String nodeID, int sequenceNum) throws DBAbstractionException;
	
	/**
	 * Gets the name of a directory at a point in time. Directories in the database do not have a name they
	 * have an id. Names are set by either the create directory event or the rename directory event. This 
	 * method gets the name at a certain point in time.
	 * 
	 * @param nodeID The node to look in
	 * @param sequenceNum The sequence number in the selected node
	 * @return The name of the directory at a point in time
	 * @throws DBAbstractionException
	 */
	//public String getDirectoryNameAtAPointInTime(String dirId, String nodeID, int sequenceNum) throws DBAbstractionException;
	...
	/**
	 * Gets all documents that occur in the list of nodeIDs. Deprecated because in the case of merge nodes, we can't be sure
	 * if a document truly exists just because it was made.  Ask PlaybackNodes and PlaybackMerge for which documents exist in them.
	 * 
	...
	/*
	@Deprecated
	public List<Document> getAllDocumentsInNodes(List<String> nodeIDs) throws DBAbstractionException;
	*/
	/**
