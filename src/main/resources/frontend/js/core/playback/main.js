...
function setupPlayback() {
	...
	(function() {
		...
		var forwardsInterval, backwardsInterval;
		function clearIntervals() {
			clearInterval(forwardsInterval);
		...
		$("#stepBack").mousedown(function() {
			...
			backwardsInterval = betterSetInterval(playback.speed, stepBackward, 1);
		...
		$("#stepForward").mousedown(function() {
			...
			forwardsInterval = betterSetInterval(playback.speed, stepForward, 1);
		}).bind("mouseup mouseleave", function() {
			// stops the above interval
			clearIntervals();
		});
...
*/
function changeDevGroup(newDeveloperGroupID) {
	// TODO: DOM access is expensive, we should store this somewhere else!
	var currentDeveloperGroupID = $("#developerPictures").data("group");

	// if the passed in developer group is NOT the same as the current dev group
	if (currentDeveloperGroupID === undefined || currentDeveloperGroupID !== newDeveloperGroupID ) {
		if (newDeveloperGroupID in developerGroups) {
			var developerGroup = developerGroups[newDeveloperGroupID];

			$("#developerPictures")
				.data("group", newDeveloperGroupID)
				.html("");

			_.forEach(developerGroup.developers, function(developerID) {
				var developer = developers[developerID];

				$("<img/>", {
					class: "devPicture",
					src: developer.gravatar,
					title: developer.firstName + " " + developer.lastName + " (" + developer.email + ")",
...
// TODO: Move the following two functions elsewhere.
//       They're only used in core/playback/storyboard.js, that's probably the
//       best place for them.
