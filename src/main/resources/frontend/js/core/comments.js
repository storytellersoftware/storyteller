...
function showComment(commentID) {
  ...
  highlightElements(playback.clipNumber + "-" + comments[commentID].startHighlightedEventID, playback.clipNumber + "-" + comments[commentID].endHighlightedEventID)
}
...
function showClipInfo(clipNumber) {
  ...
  var html = "<h2>" + storyboard.clips[clipNumber].name + "</h2>" +
             "<p>" + storyboard.clips[clipNumber].description + "</p>"

  addToCommentSection("clip-comment-" + clipNumber, "clip", html,
...
function showDescription(storyboard) {
  var html = "<h1>" + storyboard.name + "</h1>" +
             "<p>" + storyboard.description + "</p>"
  addToCommentSection("descriptionComment", "description", html,
...
function addToCommentSection(id, className, html, developerIDs) {
  ...
  newComment.animate({"background-color": "#cade97", "opacity": "1"}, {
    duration: 100, 
    easing: "linear",
