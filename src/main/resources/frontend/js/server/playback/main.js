...
var eventGrabber = {
	// where we're at in grabbing events, aka the number of events we have at the present
	currentIndex: 0,

	// have we finished grabbing all of the events
	done: false,

	// number of events to ask the server for with each request, gets smaller each
	// time the server fails to handle the request (we receive a 4xx or 5xx error)
	blockSize: 1000,

	// a date object of when we started grabbing the events, used for timing in testing
	startTime: null,

	// a date object of when we stopped grabbing the events, used for timing in testing
	endTime: null,

	// the id of the interval used for grabbing events
	poller: null,

	// is the grabbing function currently blocking, used to create a pseudo-semaphore
	blocking: false,

	// current clip being retreived from the server, only incremented when we're playing back a storyboard
	clipNumber: 0
};
